...
. Depending on which kind of Kafka you would like:
 ...
 b. Set up a Kafka instance on RHOSAK, and connect it to the `reactive-messaging-qs` application as outlined link:#install_rhosak[earlier]  in this quickstart. The `os-jar-rhosak` profile in the pom.xml triggers running the `src/main/scripts/bootable-jar/initialize-server.cli` when preparing the bootable jar, so that the config map and service binding become part of the configuration provided by the `microprofile-config-smallrye` subsystem.
endif::[]
...
b. If you are targetting Kafka supplied by RHOSAK, use the following commands.
+
