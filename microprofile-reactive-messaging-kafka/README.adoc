...
For the purposes of this Quickstart you should only select one of these, and your choice should match what we will do later in <<deploy-to-openshift, Deploy the {ProductShortName} Source-to-Image (S2I) Quickstart to OpenShift>>.
...
For the purposes of this Quickstart, you should choose one of these approaches, not both. Your choice should match the type of Kafka instance you selected earlier in <<installing-kafka-on-openshift, Installing Kafka on OpenShift>>.
...
//:reactive-messaging-amq-streams:
//:xp-deploy-project-skip-header:
//include::../shared-doc/xp-deploy-project.adoc[leveloffset=+3]
//:!xp-deploy-project-skip-header:
//:!reactive-messaging-amq-streams:
To deploy your application, make sure you have downloaded the https://docs.openshift.com/container-platform/4.8/cli_reference/helm_cli/getting-started-with-helm-on-openshift-container-platform.html[Helm CLI Tool]. `./helm-rhosak.yml` contains the information to deploy the application to be backed by Kafka provided by AMQ Streams.

// TODO talk about the contents of helm-rhosak.yml
TODO: Add description of helm-rhosak.yml
...
Then run the following command to deploy and start the application (note we are using the name `reactive-messaging-qs` for our application here:
// TODO needs a diff commit to differentiate between EAP and Wildfly

----
helm install reactive-messaging-qs -f ./helm-amq-streams.yml wildfly/wildfly
----
...
Then run the following command to deploy and start the application (note we are using the name `reactive-messaging-qs` for our application here, as mentioned when we connected RHOSAK to our OpenShift project):
// TODO needs a diff commit to differentiate between EAP and Wildfly

----
helm install reactive-messaging-qs -f ./helm-rhosak.yml wildfly/wildfly
----
The CLI command will complete fast, and after a few minutes you should see a pod for the application appear on OpenShift. Its logs will contain the output we have seen earlier.

While the OpenShift cluster is doing the work to build and deploy the application (i.e. as soon as the `helm install` has completed in your terminal), run the following command to set up the ConfigMap from the Kafka connection as mentioned earlier:
----
rhoas cluster bind --app-name reactive-messaging-qs  --binding-name kafka-config
----

// TODO might not be needed once we move both AMQ Streams and RHOSAK to Helm? I can see a route in the config
