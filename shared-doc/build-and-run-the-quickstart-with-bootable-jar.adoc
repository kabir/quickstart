...
. Depending on which kind of Kafka you would like:
 a. Install AMQ Streams into the `bootable-jar-project` OpenShift project as outlined link:#install_amq_streams[earlier]  in this quickstart. Note that the `MP_MESSAGING_CONNECTOR_SMALLRYE_KAFKA_BOOTSTRAP_SERVERS` environment variable in the `os-jar-amq-streams` profile in the pom.xml assume you have used the indicated name `my-cluster` for your AMQ Streams cluster.
 b. Set up a Kafka instance on RHOSAK, and connect it to the `reactive-messaging-qs ` application as outlined link:#install_rhosak[earlier]  in this quickstart. The `os-jar-rhosak` profile triggers running the `src/main/scripts/bootable-jar/initialize-server.cli` when preparing the bootable jar, so that the config map and service binding become part of the configuration provided by the `microprofile-config-smallrye` subsystem.
endif::[]
...
ifdef::reactive-messaging[]
The commmand to deploy the application is slightly different depending on which Kafka installation you want to target:

a. If you are targeting Kafka supplied by AMQ Streams, use the following command:
+
[source,subs="attributes+",options="nowrap"]
----
$ mvn oc:deploy -Pbootable-jar-openshift,os-jar-amq-streams
----
+
The additional `os-jar-amq-streams` profile specified as an argument to Maven sets up the `MP_MESSAGING_CONNECTOR_SMALLRYE_KAFKA_BOOTSTRAP_SERVERS` environment to connect to the cluster we specified earlier.
b. If you are targetting Kafka supplied by RHOSAK, use the following command:
+
[source,subs="attributes+",options="nowrap"]
----
$ helm install reactive-messaging-qs -f ./helm-bootable-jar-rhosak.yml wildfly/wildfly
----
+
Once the CLI returns, remember to bind the RHOSAK Kafka connection so that
