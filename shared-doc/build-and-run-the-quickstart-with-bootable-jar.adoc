...
----
ifdef::reactive-messaging[]
. Install Kafka by either:

 a. Installing AMQ Streams into the `bootable-jar-project` OpenShift project as outlined link:#install_amq_streams[earlier]  in this quickstart. Note that the `MP_MESSAGING_CONNECTOR_SMALLRYE_KAFKA_BOOTSTRAP_SERVERS` environment variable in the `os-jar-amq-streams` profile in the pom.xml assume you have used the indicated name `my-cluster` for your AMQ Streams cluster.
 b. Installing RHOSAK into the `bootable-jar-project` OpenShift project as outlined link:#install_rhosak[earlier]  in this quickstart.
...
//ifdef::reactive-messaging[]
+
The exact command depends on which Kafka installation type you want to use:
a. *AMQ Streams*
+
[source,subs="attributes+",options="nowrap"]
----
...
----
Note that the additional `os-jar-amq-streams` profile specified as an argument to Maven sets up the `MP_MESSAGING_CONNECTOR_SMALLRYE_KAFKA_BOOTSTRAP_SERVERS` environment to connect to the cluster we specified earlier. This is shown in the pom.xml snippet above.
b. *RHOSAK*
+
First deploy the Helm chart to set up the
+
[source,subs="attributes+",options="nowrap"]
----
$ mvn oc:deploy -Pbootable-jar-openshift,os-jar-rhosak
----
Note that the additional `os-jar-amq-streams` profile specified as an argument to Maven sets up the `MP_MESSAGING_CONNECTOR_SMALLRYE_KAFKA_BOOTSTRAP_SERVERS` environment to connect to the cluster we specified earlier. This is shown in the pom.xml snippet above.
