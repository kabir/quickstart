...
Copy across the `microprofile-reactive-messaging-kafka/src/main/java/org/jboss/as/quickstarts/reactive/messaging/MockExternalAsyncResource.java` file to your project. This class mocks a call to an asynchronous external resource. The details of how it is implemented are not important for this QuickStart.
