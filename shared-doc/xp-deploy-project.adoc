...
ifdef::reactive-messaging-rhosak[]
 ...
 -e APPLICATION_NAME=reactive-messaging-qs {backslash}
endif::[]
ifndef::uses-jaeger[]
 ...
 * `-p APPLICATION_NAME=reactive-messaging-qs` - This names the application `reactive-messaging-qs` so it matches the earlier service binding we created.
endif::[]
